...
MonoTopSkim::MonoTopSkim(const edm::ParameterSet &iConfig)
    : // now do what ever initialization is needed
      ...
      EDMPhotonsToken{consumes<std::vector<pat::Photon>>(
          iConfig.getParameter<edm::InputTag>("photons"))},
      EDMAK4JetsToken{consumes<pat::JetCollection>(
          iConfig.getParameter<edm::InputTag>("AK4jets"))},
      EDMAK8JetsToken{consumes<pat::JetCollection>(
          iConfig.getParameter<edm::InputTag>("AK8jets"))},
      EDMMETToken{consumes<std::vector<pat::MET>>(
          iConfig.getParameter<edm::InputTag>("met"))},
      EDMPuppiMETToken{consumes<std::vector<pat::MET>>(
          iConfig.getParameter<edm::InputTag>("met_puppi"))},
      ...
      EDMAK15JetsToken_{consumes<pat::JetCollection>(
          iConfig.getParameter<edm::InputTag>("AK15jets"))},
...
bool MonoTopSkim::filter(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  auto met_puppi_up =
      hPuppiMETs->at(0).shiftedP4(pat::MET::JetEnUp, pat::MET::Type1);
  auto met_puppi_down =
      hPuppiMETs->at(0).shiftedP4(pat::MET::JetEnDown, pat::MET::Type1);
  ...
  auto met_puppi_max = std::max(
      met_puppi.pt(), std::max(met_puppi_up.pt(), met_puppi_down.pt()));
  ...
  if (met_criterium && jet_criterium)
    return true;
  ...
  selectedElectrons.erase(
      std::remove_if(selectedElectrons.begin(), selectedElectrons.end(),
                     [&](pat::Electron ele) {
                       return (ele.pt() < electronPtMin_ ||
                               fabs(ele.eta()) > electronEtaMax_ ||
                               !ele.electronID(
  ...
  selectedMuons.erase(std::remove_if(selectedMuons.begin(), selectedMuons.end(),
                                     [&](pat::Muon mu) {
                                       return (
                                           mu.pt() < muonPtMin_ ||
                                           fabs(mu.eta()) > muonEtaMax_ ||
                                           !muon::isLooseMuon(mu) ||
                                           !mu.passed(pat::Muon::PFIsoLoose));
                                     }),
                      selectedMuons.end());
  ...
  selectedPhotons.erase(
      std::remove_if(
          ...
          [&](pat::Photon ph) {
            return (ph.pt() < photonPtMin_ || fabs(ph.eta()) > photonEtaMax_ ||
                    !ph.photonID("cutBasedPhotonID-Fall17-94X-V2-loose"));
          }),
      selectedPhotons.end());

  // std::cout << "Number of selected electrons: " << selectedElectrons.size()
  // << std::endl;
  // std::cout << "Number of selected muons: " << selectedMuons.size() <<
  // std::endl;
  // std::cout << "Number of selected photons: " << selectedPhotons.size() <<
  // std::endl;
  ...
  auto hadr_recoil_max = std::max(
      hadr_recoil.pt(), std::max(hadr_recoil_up.pt(), hadr_recoil_down.pt()));
  auto hadr_recoil_puppi_max =
      std::max(hadr_recoil_puppi.pt(), std::max(hadr_recoil_puppi_up.pt(),
                                                hadr_recoil_puppi_down.pt()));

  // check if we have sizeable hadronic recoil in the event
  bool recoil_criterium =
      (hadr_recoil_max >= metPtMin_) || (hadr_recoil_puppi_max >= metPtMin_);

  // keep the event if recoil and fatjet criteria are fulfilled
  if (recoil_criterium && jet_criterium)
    return true;
  ...
  // for leptonic monotop events, dont use ID criteria because of included
  // isolation cuts
  selectedElectrons.erase(
      std::remove_if(selectedElectrons.begin(), selectedElectrons.end(),
                     [&](pat::Electron ele) {
                       return (ele.pt() < (electronPtMin_ + 10.) ||
                               fabs(ele.eta()) > electronEtaMax_);
                     }),
      selectedElectrons.end());
  ...
  // [&](pat::Jet jet) { return (CSVHelper::PassesCSV(jet, "DeepJet",
  // CSVHelper::CSVwp::Loose, era)); });

  int n_harder_jets =
      std::count_if(ak4Jets->begin(), ak4Jets->end(),
                    [&](pat::Jet jet) { return (jet.pt() >= 30.); });
  // std::cout << "Number of loosely btagged jets: " << n_btagged_jets <<
  // std::endl;

  // leading lepton pts
  // auto leading_ele    = n_electrons > 0 ? selectedElectrons.at(0).p4() :
  // math::XYZTLorentzVector(0., 0., 0., 0.);
  // auto leading_muon   = n_muons > 0 ? selectedMuons.at(0).p4() :
  // math::XYZTLorentzVector(0., 0., 0., 0.);
  // auto leading_lepton = leading_ele.pt() > leading_muon.pt() ? leading_ele :
  // leading_muon;
  auto leading_jet_pt = n_ak4jets > 0 ? ak4Jets->at(0).pt() : 0.;

  // auto cos_dphi_met_lep       =
  // TMath::Cos(fabs(TVector2::Phi_mpi_pi(met.phi() - leading_lepton.phi())));
  // auto m_W_transv             = TMath::Sqrt(2 * leading_lepton.pt() *
  // met.pt() * (1 - cos_dphi_met_lep));
  // auto cos_dphi_met_lep_puppi =
  // TMath::Cos(fabs(TVector2::Phi_mpi_pi(met_puppi.phi() -
  // leading_lepton.phi())));
  // auto m_W_transv_puppi       = TMath::Sqrt(2 * leading_lepton.pt() *
  // met_puppi.pt() * (1 - cos_dphi_met_lep_puppi));

  // criterium which lowers requested MET value for events in the leptonic
  // channel
  bool lepton_jet_met_criterium = (n_leptons >= 1) && (n_ak4jets > 0) &&
                                  (leading_jet_pt > 50.) &&
                                  (met_max > 80. || met_puppi_max > 80.);
...
// module  ------------
void MonoTopSkim::fillDescriptions(
